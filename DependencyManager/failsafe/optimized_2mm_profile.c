...
__attribute__((aligned(64))) float F[NLV][NLV],G[NLV][NLV],H[NLV][NLV],I[NLV],J[NLV],K[NLV],L[NLV];
...
void mm2_OMP(DATA_TYPE* A, DATA_TYPE* B,DATA_TYPE* D)
{
   ...
   #pragma omp target data map(to:A[0:1024]) map(tofrom:D[0:1024]) map(to:B[0:1024])
...
#pragma omp simd simdlen(8) aligned(H,G,F:64)
for (int i = 0; i < NLV; i++)
    {
...
#pragma omp simd simdlen(8) aligned(I,K,J,L:64)
for (int i = 0; i < NLV; i++)
    {
