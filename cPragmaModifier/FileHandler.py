...
class FileHandler:
    ...
    def readDirectives(self):
        directiveDataDict = {}
        directivesList = json.load(open(os.path.dirname(os.path.realpath(__file__))+"/Directives/main.json"))
        directivesList = directivesList["directives"].split(",")
        for directive in directivesList:
            filePath = os.path.dirname(os.path.realpath(__file__))+"/Directives/" + directive + ".json"
