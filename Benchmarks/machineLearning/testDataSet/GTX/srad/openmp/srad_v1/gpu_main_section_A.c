...
#pragma omp target data map(to:image[0:Nr*Nc]) map(tofrom:dN[0:Nr*Nc]) map(tofrom:dS[0:Nr*Nc]) map(tofrom:dW[0:Nr*Nc]) map(tofrom:dE[0:Nr*Nc])  map(tofrom:c[0:Nr*Nc]) map(to:iN[0:Nr]) map(to:iS[0:Nr]) map(to:jW[0:Nc]) map(to:jE[0:Nc])
#pragma omp target teams
#pragma omp distribute parallel for collapse(2) shared(image, dN, dS, dW, dE, c, Nr, Nc, iN, iS, jW, jE) private(i, j, k, Jc, G2, L, num, den, qsqr)
