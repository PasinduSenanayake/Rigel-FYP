...
#pragma omp parallel
{
  ...
  for (int i=0; i<N; i++)
  {
    for (int j=0; j<N; j++)
    {
      ...
      x2[i] = x2[i] + a[j*N + i] * y2[j]; //moved here
    }
  }
/*#pragma omp for --> before
  for (int i=0; i<N; i++)
  {
    for (int j=0; j<N; j++)
    {
...
void runMvt_OMP(DATA_TYPE* a, DATA_TYPE* x1, DATA_TYPE* x2, DATA_TYPE* y1, DATA_TYPE* y2)
{
  ...
  #pragma omp target data map(to: a[:N*N], y1[:N], y2[:N]) map(tofrom: x1[:N], x2[:N])
  {
    #pragma omp target teams thread_limit(128)
    {
      ...
      for (int i=0; i<N; i++)
      {
        ...
        {
          x1[i] = x1[i] + a[i*N + j] * y1[j];
          x2[i] = x2[i] + a[j*N + i] * y2[j]; //moved here and made a single for loop
        }
      }
    }
}
/*
      //#pragma omp distribute parallel for ---> before
      for (int i=0; i<N; i++)
      {
        for (int j=0; j<N; j++)
        {
