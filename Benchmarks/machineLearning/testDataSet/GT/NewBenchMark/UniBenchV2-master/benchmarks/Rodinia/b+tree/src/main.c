...
#include <unistd.h>
#include <sys/time.h>
#include "./rodiniaUtilFunctions.h"
...
void compareResults(long *offset_cpu, long *offset_gpu, long *currKnode_cpu, long *currKnode_gpu, record *ans_cpu, record *ans_gpu, count) {
  ...
  for (i=0; i<count; i++) {
      if (percentDiff(offset_cpu[i], offset_gpu[i]) > ERROR_THRESHOLD) {
	fail++;
      }
      if (percentDiff(currKnode_cpu[i], currKnode_gpu[i]) > ERROR_THRESHOLD) {
	fail++;
      }
      if (percentDiff(ans_cpu[i].value, ans_gpu[i].value) > ERROR_THRESHOLD) {
	fail++;
      }
  }
	  ...
	  commandPointer++;
		switch (instruction) {
			...
			case 'k':
			{
				...
				long *currKnode_cpu;
				currKnode_cpu = (long *)malloc(count*sizeof(long));
				// INPUT: offset CPU initialization
				memset(currKnode_cpu, 0, count*sizeof(long));

				long *currKnode_gpu;
				currKnode_gpu = (long *)malloc(count*sizeof(long));
				...
				record *ans_cpu = (record *)malloc(sizeof(record)*count);
				// OUTPUT: ans CPU initialization
				for(i = 0; i < count; i++){
					ans_cpu[i].value = -1;
				}

				record *ans_gpu = (record *)malloc(sizeof(record)*count);
				// OUTPUT: ans CPU initialization
				for(i = 0; i < count; i++){
					ans_gpu[i].value = -1;
				}
