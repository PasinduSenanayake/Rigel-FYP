...
int main(int argc, char *argv []){
	...
	read_graphics(	"../../data/srad/galaxy.pgm",
								image_ori,
								image_ori_rows,
								image_ori_cols,
								1);
...
//Section A
	...
	//	#pragma omp parallel for shared(image, dN, dS, dW, dE, c, Nr, Nc, iN, iS, jW, jE) private(i, j, k, Jc, G2, L, num, den, qsqr)
		...
		//#pragma omp parallel for shared(image, c, Nr, Nc, lambda) private(i, j, k, D, cS, cN, cW, cE) num_teams(Nr*Nc/512) thread_limit(512)
	double startTime = omp_get_wtime();

		#pragma omp target data map(tofrom:image[0:Nr*Nc]) map(to:dN[0:Nr*Nc]) map(to:dS[0:Nr*Nc]) map(to:dW[0:Nr*Nc]) map(to:dE[0:Nr*Nc])  map(to:c[0:Nr*Nc]) map(to:iS[0:Nr]) map(to:jE[0:Nc])
		#pragma omp target teams thread_limit(640)
		#pragma omp distribute parallel for collapse(2) schedule(static) shared(image, c, Nr, Nc, lambda) private(i, j, k, D, cS, cN, cW, cE)
        for (j=0; j<Nc; j++) {												// do for the range of columns in IMAGE
